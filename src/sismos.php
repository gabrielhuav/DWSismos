<?php
    ...
    // Coordenadas aproximadas de los estados
    $state_coordinates = [
    ...
    if (!$stmt_zonas) {
        ...
        if (!$result_zonas) {
            ...
            while ($row = pg_fetch_assoc($result_zonas)) {
                if (isset($state_coordinates[$row['nom_ent']])) {
                    $row['latitud'] = $state_coordinates[$row['nom_ent']]['latitud'];
                    $row['longitud'] = $state_coordinates[$row['nom_ent']]['longitud'];
                    $zonas[] = $row;
                }
            }
    ...
    if (sismosData.length > 0) {
        ...
        sismosData.forEach(function(sismo) {
            if (sismo.latitud && sismo.longitud) {
                bounds.push([sismo.latitud, sismo.longitud]);
            }
        });
        if (hasZonas) {
            zonasData.forEach(function(zona) {
                if (zona.latitud && zona.longitud) {
                    bounds.push([zona.latitud, zona.longitud]);
                }
            });
        }
        if (bounds.length > 0) {
